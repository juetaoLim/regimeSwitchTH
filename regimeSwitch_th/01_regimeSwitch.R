...
#THIS ESTIMATES THE REGIME SWITCHING MODEL
#THIS ESTIMATES THE REGIME SWITCHING MODEL
#THIS ESTIMATES THE REGIME SWITCHING MODEL
#THIS ESTIMATES THE REGIME SWITCHING MODEL
...
for (j in 1:ncol(df)){
  ...
  postProbs <- s1/(bmr)
  plot(y=postProbs,x=1:length(postProbs),main = c(colnames(df)[j]),type="l")
  filename <- paste("~/nBox/regime_switch_hamfil_thailand/results/",colnames(df)[j],".Rdata",sep="")
  save.image(filename)
  postProbs[which(postProbs>0.5)] <- 1
  postProbs[which(postProbs<0.5)] <- 0
  plot.ts(cbind(postProbs,df[,j]))
}